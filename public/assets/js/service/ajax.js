...
AjaxSend.prototype.send = function(configuration) {
    ...
    $.ajax({
            ...
            if (settings.show === undefined) {
                if (!settings.block) {
                    if (settings.target !== undefined) {
                        settings.target.block({message: $('#loader-modal').html(), baseZ: 9999});
                    } else {
                        $.blockUI({message: $('#loader-modal').html(), baseZ: 9999});
                    }
                }
            } else {
                settings.show();
        ...
        if (settings.error !== undefined)  {
            ...
            window.photoevent.dialog.error('Erro', jqXHR.responseJSON);
        }
    });
...
window.photoevent.ajax = window.photoevent.ajax || new AjaxSend();